...
Common:
		Commonly used by postal senders

Name:	
		Definition of abbreiviated suffix as complete word
		used for primary street name (address line 1)

Abbreviated (USPS):
		Official suffix used in address automation by USPS



ABOUT US POSTAL CODES

Us postal codes sourced from
https://www.aggdata.com/node/86
which was sourced from : http://www.geonames.org/


ABOUT US GOV. CENSUS DATA
...
In the file  "dist.all.last" one sample entry appears as:
	
	MOORE 0.312 5.312 9

	MOORE ranks 9th in terms of frequency. 
	5.312 % of the sample population is covered by MOORE 
	and the 8 names occurring more frequently than MOORE. 
	
	The surname, MOORE, is possessed by 0.312 % of our population sample.
...
Each of the three files, (all.last.txt), (male.first.txt), and
(female.first.txt) contain four items of data.  The four items
are:	
...
In the file (all.last.txt) one entry appears as:
...
up the unabridged files are: 
  ...
  1. all.last.txt			6.290,251			 88,799 
  2. female.first.txt		3,184,399			  4,275
  3. male.first.txt			3,003,954			  1.219 
...
Consider the entry JOSEHP in "male.first.txt".  Although JOSEHP
...
file all.last.txt  Here the frequency would appear as 0.001
...
(male.first.txt).  Although fathers and one son may share a
